...
words <- c("Real", "Madrid", "es", "muy", "bien", "club")
...
# of the word!
first_letter <- substr(tolower(words), 1, 1)
...
word_bin <- function(words, letter1, letter2) {
  first_letters <- substr(tolower(words), 1, 1)
  out <- words[first_letters >= letter1 & first_letters <= letter2]
  out 
}
