...
by_type <- avocados %>% 
  group_by(Date, type) %>%
  summarise(Total.vol = sum(Total.Volume))
...
# the `spread()` function!
by_type_wide <- spread(by_type, type, Total.vol)
