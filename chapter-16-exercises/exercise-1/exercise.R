...
ggplot(diamonds_sample) +
  geom_point(aes(x=price, y=carat, color=cut), alpha=0.3) +
  geom_smooth(aes(x=price, y=carat, color=cut), se = FALSE)
...
# mean+error.
ggplot(clarity_summary, aes(x=clarity, y=avg_price)) + 
  geom_bar(mapping = aes(fill = clarity), stat = "identity") +
  geom_errorbar(mapping = aes(ymin = (avg_price - std_error), ymax = (avg_price + std_error)))
